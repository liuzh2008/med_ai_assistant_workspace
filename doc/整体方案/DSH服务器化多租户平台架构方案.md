...
> 状态：方案讨论稿 v1.3（2026-09-07，grill-with-docs 审查修复：MedAi /api/auth/* 后端缺失漂移、启动产物 bin.js、§4.6.3 与 §5 容量口径矛盾、§7 编号错位；v1.2 规模收敛；v1.1 硬件实测）
...
| 100.66.1.4（Linux 形态验证） | 联想小新 Pro 14：i5-13500H / **16GiB（可用仅 ~3.8GiB）** / NVMe 余 379GB | **LPDDR5 板载焊接不可扩内存**；swap 4GB 已耗尽、Committed_AS 超卖；7 容器占 ~5GB（execution 1.58/8GB 未紧限、oracle 1.61/3GB、main 1.34/**未限**） |
...
| 100.66.1.4（Linux 形态验证） | **形态验证小池 2-4**（限容后可试 8 上限，有 OOM 风险） | 4×0.35 + 4×0.15 + 网关 0.4 ≈ **2.4GB** | 现状可用 ~3.8GB：启用前给 med-ai-main 限容并重启回收缓存（§5）；**几十人试点不可行** |
...
2. **试点主环境 = 本机 Windows**（资源现成、MedAi 后端 127.0.0.1:8081 同机、联调链路最短）；100.66.1.4 降为 Linux 形态验证（网络隔离/容器化）——其内存不足，验证前需容器限容或加内存，或仅跑小池。
...
| 并发池 | 16-32（可用内存允许开到 60-80 压测） | 仅形态验证小池 2-4（实测核定：LPDDR5 板载不可扩内存、swap 耗尽、超卖；几十人试点不可行——OOM 会误伤 MedAi 容器） |
| 用途 | 阶段 0 原型 + 阶段 1 几十人试点主力 | Linux 形态验证（网络隔离/容器化）与正式部署样板；启用前动作：给 med-ai-main 限容（当前未限 15.34GB）并重启回收缓存，挤出 1-2GB |
...
交付：supervisor 池正式化（开户/启停/回收/审计/管理页）+ watchdog 常驻；用户 provisioning 自动化（MedAi 账号同步或首登开户）；3-5 真实用户试用（池 16-32）；**同时在 100.66.1.4 做 Linux 形态验证**（容器化 + iptables/容器隔离——验证前需容器限容或加内存）。
