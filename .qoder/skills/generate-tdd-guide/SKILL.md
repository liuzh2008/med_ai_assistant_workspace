...
代码可以完全重写，测试不应受影响。好的测试关注系统"做什么"，而不是"怎么做"。
...
|------|--------|--------|
| **关注点** | 外部可观测行为 | 内部实现细节 |
| **读起来像** | 规格说明（"用户可用有效购物车结账"） | 代码调用序列 |
| **重构后** | 仍然通过（行为不变） | 失败（内部变了） |
| **Mock对象** | 仅 mock 外部边界（HTTP、DB） | mock 内部协作者 |
| **测试什么** | 公共 API/接口 | 私有方法、内部状态 |

> 警示信号：重命名内部函数后测试失败——说明测试测的是实现，不是行为。

### 反模式：水平切片

**禁止**：先一次性写完所有测试，再一次性实现所有代码。这是"水平切片"——把 RED 阶段当作"写所有测试"，GREEN 当作"写所有代码"。

水平切片的问题：
- 批量编写的测试测的是**想象的行为**，而非实际行为
- 测试关注数据结构和函数签名，而非用户可观测行为
- 测试对真实变化不敏感——行为破坏时测试仍通过，行为正确时测试反而失败
- 在理解实现之前就锁定了测试结构

**正确做法：垂直切片 + 示踪弹（Tracer Bullet）**
...
正确（垂直）:
RED→GREEN: test1→impl1  ← 示踪弹：证明路径可用
RED→GREEN: test2→impl2  ← 增量循环
RED→GREEN: test3→impl3  ← 每次学到的反馈驱动下一步
```
...
- 验收标准必须**可测试**（有明确的验证方法）
- **必须按垂直切片拆分**：每个用户故事是一个端到端功能，而非按技术层（前端/后端/数据库）
- **禁止水平切片**：不要将"写所有前端"作为一个用户故事，"写所有后端"作为另一个——测试会变成对数据结构的验证而非行为的验证
...
> - **每个周期只测一个行为**，一次 RED→GREEN 只处理一个测试

#### TDD 行为核实清单

在每个 RED→GREEN→REFACTOR 周期执行前，确认以下事项：
...
每个周期遵循**示踪弹（Tracer Bullet）→ 增量循环**模式：
1. **C1（示踪弹）**：编写第一个测试，最小代码让它通过——证明路径可用
2. **C2+（增量循环）**：逐个添加行为，每步从上一个周期学到的反馈驱动

> 每次只修改一个变量：一个测试 → 一个实现 → 验证 → 下一个。禁止一次性处理多个测试。

创建周期跟踪表格：

| 周期 | 任务描述 | 红（失败测试） | 绿（最小实现） | 重构 | 预估时间 | 状态 |
|------|---------|---------------|---------------|------|---------|------|
| C1（示踪弹） | [第一个端到端行为] | [测试类名] | [实现类/方法] | [重构点] | [时间] | ⬜ |
| C2 | [下一个行为] | [测试类名] | [实现类/方法] | [重构点] | [时间] | ⬜ |
| C3 | [再下一个行为] | [测试类名] | [实现类/方法] | [重构点] | [时间] | ⬜ |
