...
- 网络与HTTP
  - Apache HttpClient 5.x + 连接池，支持Keep-Alive与重试拦截器
...
图表来源
- [主服务器架构总览.md:9-67](file://med_ai_assistant_1.0_bs_backend/doc/其他/主服务器架构总览.md#L9-L67)
- [主服务器业务功能模块分析.md:85-83](file://med_ai_assistant_1.0_bs_backend/doc/其他/主服务器业务功能模块分析.md#L85-L83)
...
图表来源
- [application.properties:30-58](file://med_ai_assistant_1.0_bs_backend/src/main/resources/application.properties#L30-L58)
...
### HTTP连接池与重试策略
- Apache HttpClient 5.x连接池：最大总连接、每路由最大连接、请求/连接/Socket超时、Keep-Alive
...
- 运行时依赖：Spring Boot Starter（WebFlux、Data JPA、Actuator、Validation），Oracle/MySQL驱动，Micrometer/Prometheus，Argon2，Lombok，Apache HttpClient 5.x
...
POM --> HTTP["Apache HttpClient 5.x"]
...
## 结论
后端架构以Spring Boot 3.5.8 + JDK 21为核心，结合HikariCP、Apache HttpClient 5.x、Micrometer/Prometheus/Actuator，形成高并发、高可用、高安全的现代化企业级后端。通过主-执行服务器分离、状态机数据流与完善的监控告警体系，系统在医疗场景下具备良好的稳定性与可维护性。建议持续引入Redis缓存、容器化部署、微服务化演进与分布式追踪，进一步提升性能与可观测性。
