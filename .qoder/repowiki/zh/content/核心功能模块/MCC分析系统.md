...
- 新增模板优化接口的技术实现细节和使用说明
- 更新依赖关系分析，反映新增的模板管理功能
...
7. [MCC分析Prompt模板优化](#mcc分析prompt模板优化)
8. [依赖关系分析](#依赖关系分析)
9. [性能考虑](#性能考虑)
10. [故障排除指南](#故障排除指南)
11. [结论](#结论)
...
**新增** 系统提供了管理员权限的Prompt模板管理功能：

- **模板更新**：`POST /api/admin/prompt-templates/mcc-analysis`
- **模板查询**：`GET /api/admin/prompt-templates/{templateId}`
- **模板列表**：`GET /api/admin/prompt-templates`
- **模板删除**：`DELETE /api/admin/prompt-templates/{templateId}`
...
subgraph "服务层面"
J[MccScreeningService] --> K[相似度计算]
J --> L[排除规则检查]
J --> M[Top-K控制]
N[TimerPromptGenerator] --> O[批量患者处理]
N --> P[串行Prompt生成]
N --> Q[MCC筛选集成]
N --> R[性能测试覆盖]
S[PromptTemplateService] --> T[模板内容管理]
end
subgraph "数据访问层"
U[DrgMccRepository] --> V[数据库]
W[DiagnosisRepository] --> V
X[PromptTemplateRepository] --> V
Y[PromptRepository] --> V
Z[PatientRepository] --> V
AA[SequenceConsistencyService] --> BB[序列检查]
end
subgraph "工具层"
CC[LevenshteinUtil] --> DD[文本规范化]
CC --> EE[相似度算法]
...
**新增章节** 系统提供了管理员权限的Prompt模板优化功能，支持动态更新MCC分析模板内容，放宽排除标准，强调"宁可多保留，不要漏掉"的临床原则。

### 模板优化功能概述

管理员可以通过专门的接口更新"合并症或并发症分析"Prompt模板，优化MCC筛选的排除标准：

```mermaid
flowchart TD
A[管理员请求模板更新] --> B[验证权限和参数]
B --> C{参数有效?}
C --> |否| D[返回错误响应]
C --> |是| E[查询现有模板]
E --> F{模板存在?}
...
**新增** 管理员模板管理接口提供了完整的接口文档：

| 接口路径 | 方法 | 功能描述 | 请求参数 | 响应示例 |
|----------|------|----------|----------|----------|
| `/api/admin/prompt-templates/mcc-analysis` | POST | 更新MCC分析Prompt模板 | templateName: 合并症或并发症分析<br/>promptContent: 优化后的模板内容<br/>updateReason: 更新原因说明 | 成功响应包含模板ID、更新时间、版本号 |
| `/api/admin/prompt-templates/{templateId}` | GET | 查询指定模板详情 | templateId: 模板ID | 返回模板详细信息 |
| `/api/admin/prompt-templates` | GET | 获取所有激活的模板列表 | 无 | 返回模板列表 |
| `/api/admin/prompt-templates/{templateId}` | DELETE | 删除指定模板 | templateId: 模板ID | 返回删除成功状态 |

### 排除标准优化

**更新** 系统的排除规则标准已放宽，强调临床实用性：

- **必须排除**：与主要诊断完全相同的并发症或合并症
- **必须排除**：与病人实际情况明显矛盾的诊断（如男性病人的妊娠相关并发症）
- **谨慎排除**：与主要诊断高度相似且临床意义重复的诊断
- **保留**：相关但不完全相同的诊断（如冠心病患者的动脉硬化）

### 优化后的模板结构

```mermaid
flowchart TD
A[合并症或并发症分析模板] --> B[1. 排除原则]
B --> C[必须排除的情况]
C --> D[与主要诊断完全相同]
D --> E[与病人实际情况明显矛盾]
E --> F[谨慎排除的情况]
...
**图表来源**
- [MCC分析Prompt模板优化接口.md:31-51](file://med_ai_assistant_1.0_bs_backend/doc/接口/DRG分析/MCC分析Prompt模板优化接口.md#L31-L51)

### 模板管理服务实现

**新增** PromptTemplateService实现了模板管理的核心逻辑：

```mermaid
sequenceDiagram
participant Admin as 管理员
participant Controller as AdminPromptTemplateController
participant Service as PromptTemplateService
participant Repo as PromptTemplateRepository
participant Cache as 模板缓存
Admin->>Controller : POST /api/admin/prompt-templates/mcc-analysis
Controller->>Controller : 参数验证
Controller->>Service : updateMccAnalysisTemplate(templateName, promptContent, updateReason)
Service->>Repo : findByPromptTypeAndPromptName("其他", "合并症或并发症分析")
Repo-->>Service : 返回模板记录
Service->>Service : 更新Prompt内容和元数据
Service->>Cache : 刷新模板缓存
Service->>Service : 记录更新日志
Service-->>Controller : 返回更新结果
Controller-->>Admin : 返回成功响应
```

**图表来源**
- [MCC分析Prompt模板优化接口.md:74-79](file://med_ai_assistant_1.0_bs_backend/doc/接口/DRG分析/MCC分析Prompt模板优化接口.md#L74-L79)

### 数据模型支持

**新增** PromptTemplate实体支持模板内容的动态更新：

- **promptId**: 模板唯一标识符
- **promptType**: 模板类型（如"其他"）
- **promptName**: 模板名称（如"合并症或并发症分析"）
- **prompt**: 模板内容（支持动态更新）
- **filterRules**: 过滤规则（可选）
- **specialContent**: 特殊内容（可选）
- **requiredDataTypes**: 所需数据类型
- **scope**: 适用范围
- **departmentId**: 科室ID
- **isActive**: 是否激活
- **版本控制**: 支持模板版本追踪
...
+updateMccAnalysisTemplate(request) ResponseEntity
+getPromptTemplate(templateId) ResponseEntity
+getAllPromptTemplates() ResponseEntity
}
class PromptTemplateService {
+updateMccAnalysisTemplate(templateName, promptContent, updateReason) PromptTemplate
+getPromptTemplateById(templateId) PromptTemplate
+getAllActivePromptTemplates() PromptTemplate[]
}
class MccCandidate {
+String mccCode
+String mccName
...
+findByIsActive(isActive) PromptTemplate[]
}
...
PromptTemplateController --> PromptTemplateService : 依赖
PromptTemplateService --> PromptTemplateRepository : 依赖
...
- [MCC分析Prompt模板优化接口.md:111-115](file://med_ai_assistant_1.0_bs_backend/doc/接口/DRG分析/MCC分析Prompt模板优化接口.md#L111-L115)
...
- 验证编码匹配的大小写处理
- **新增** 检查模板内容，确认排除标准已更新为"明显不可能"的判断
...
- 运行序列一致性检查
- 查看详细错误日志
- **新增** 检查模板缓存状态，必要时手动刷新缓存

#### 5. 模板更新接口失败

**问题表现**：管理员无法更新MCC分析Prompt模板
...
**解决方案**：
- 确认管理员身份验证
- 检查请求参数格式（templateName必须为"合并症或并发症分析"）
- 验证promptContent非空且格式正确
- 检查数据库连接状态
- 查看详细错误日志
- **新增** 确认模板缓存已正确刷新
...
- [MCC分析Prompt模板优化接口.md:54-70](file://med_ai_assistant_1.0_bs_backend/doc/接口/DRG分析/MCC分析Prompt模板优化接口.md#L54-L70)
...
**新增的管理员Prompt模板优化功能**为系统带来了显著的价值：

- **动态模板管理**：管理员可随时更新MCC分析模板内容
- **临床导向优化**：放宽排除标准，强调"宁可多保留，不要漏掉"原则
- **智能排除规则**：仅排除明显不可能的MCC，保留相关但不完全相同的诊断
- **版本控制**：支持模板版本追踪和回滚
- **缓存管理**：支持模板更新后的缓存刷新
...
**更新的排除规则标准**进一步提升了系统的临床实用性：

- **放宽排除标准**：仅排除明显不可能的MCC，减少误诊风险
