...
## 更新摘要
**所做更改**
- 新增治疗计划连续性功能模块，包括数据库表创建、CRUD接口、前端组件等完整实现
- 新增TREATMENT_PLAN_ITEMS表结构和相关SQL脚本
- 新增治疗计划项目管理的完整API接口和业务逻辑
- 新增前端治疗计划表格组件和解析工具
- 实现诊疗计划的结构化存储、编辑、保存和连续性查询功能
...
**更新重点**：本次更新重点新增了治疗计划连续性功能模块，该功能实现了诊疗计划表的结构化存储与连续性迭代，包括数据库表创建、CRUD接口、前端组件等完整功能实现，支持AI生成的Markdown诊疗计划表解析为结构化项目数据，前端提供可编辑表格，支持用户对每条项目进行确认、修改、删除和保存，并在下次生成时从结构化表中提取上次计划内容作为上下文。
...
**更新重点**：架构中新增了Prompt提交与数据取回机制的加固层，包括事务一致性保障、乐观锁重试机制和JPQL直接更新策略，确保数据处理的可靠性和一致性。**新增**：软删除机制确保用户操作与系统自动修复之间的平衡。**新增**：治疗计划连续性功能模块，包括数据库表结构、API接口、业务逻辑和服务层实现。
...
### 数据模型关系
系统围绕患者、诊断、病历、化验、检查、提示词与结果、语音识别、**治疗计划项目**等核心实体建立ER关系，支持跨表关联与查询。
...
## 依赖关系分析
- 控制器依赖：AIController依赖多个Repository与服务类，形成清晰的业务编排层；新增VoiceFileRecognitionController处理语音识别功能；**新增**：AIController中新增治疗计划项目管理接口
- 配置依赖：异步线程池与调度参数通过配置类集中管理，便于横向扩展；新增VoiceRecognitionProperties配置类管理ASR服务配置；**新增**：PromptServiceConfig集中管理Prompt服务配置参数
- 数据访问：Repository层统一抽象，底层数据源支持MySQL与Oracle；**新增**：TreatmentPlanItemRepository处理治疗计划项目数据访问
...
系统在数据访问、线程池并发、数据库连接池与AI模型健康检查等方面进行了针对性优化，具备良好的稳定性与扩展性。**新增**的软删除机制确保了用户操作与系统自动修复之间的平衡，为后续的功能扩展奠定了坚实的基础。**新增**的治疗计划功能模块进一步增强了系统的临床应用价值，为医生提供更加智能化的诊疗计划管理工具。
