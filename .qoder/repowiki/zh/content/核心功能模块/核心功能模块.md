...
## 更新摘要
**所做更改**
- 新增Prompt轮询服务Bug修复说明，解决用户删除AI结果后自动回填ResultId的问题
- 更新Prompt数据一致性保障机制，包括软删除机制和ResultId关联保护
- 新增countDeletedByPromptId方法的使用说明，区分用户主动删除与数据不一致情况
- 更新数据一致性检查机制，增强用户操作与系统自动修复的平衡
...
**更新重点**：本次更新重点关注版本0.7.031中Prompt轮询服务的Bug修复，该修复解决了用户删除AI结果后自动回填ResultId的问题，通过新增的软删除机制和ResultId关联保护，提升了数据一致性和用户体验。
...
**更新重点**：架构中新增了Prompt提交与数据取回机制的加固层，包括事务一致性保障、乐观锁重试机制和JPQL直接更新策略，确保数据处理的可靠性和一致性。**新增**：软删除机制确保用户操作与系统自动修复之间的平衡。
...
系统在数据访问、线程池并发、数据库连接池与AI模型健康检查等方面进行了针对性优化，具备良好的稳定性与扩展性。**新增**的软删除机制确保了用户操作与系统自动修复之间的平衡，为后续的功能扩展奠定了坚实的基础。
