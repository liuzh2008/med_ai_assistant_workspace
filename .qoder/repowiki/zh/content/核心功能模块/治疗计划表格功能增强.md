...
## 更新摘要
**变更内容**
- 新增"加入待办"按钮功能，支持将治疗计划项一键添加为待办事项
- 新增后端API接口`/api/medicalrecords/todos/from-treatment-plan`
- 新增数据传输对象`CreateTodoFromTreatmentPlanDTO`
- 新增完整的E2E测试覆盖
- 增强待办事项集成功能
- **新增**：版本0.8.050集成QC质量控制评估数据，治疗计划表模板更新后支持'质控'优先级选项，质控项目享有更高的执行优先级
...
Markdown解析器负责将AI生成的非结构化文本转换为结构化的表格数据：

#### 解析流程

```mermaid
flowchart TD
A[接收Markdown内容] --> B[移除thinking标签]
B --> C[按行分割内容]
C --> D[收集表格行]
D --> E{找到有效表格?}
E --> |否| F[回退查找表头]
...
**图表来源**
- [treatmentPlanParser.js:144-314](file://med_ai_assistant_1.0_bs_vue/src/utils/treatmentPlanParser.js#L144-L314)

#### 支持的表格格式

| 列数 | 列标题 | 说明 | 示例 |
|------|--------|------|------|
| 4列 | 项目类型<br>项目描述<br>注意事项<br>重要程度 | 首次生成的标准格式 | 基础诊疗计划 |
| 5列 | 项目类型<br>项目描述<br>注意事项<br>重要程度<br>上次计划 | 包含对比信息的增强格式 | 增量分析版本 |

**章节来源**
- [treatmentPlanParser.js:114-143](file://med_ai_assistant_1.0_bs_vue/src/utils/treatmentPlanParser.js#L114-L143)

### API接口设计

系统提供完整的RESTful API来支持治疗计划表格的各种操作：

#### 核心API接口

| 接口名称 | 方法 | 路径 | 功能描述 |
|----------|------|------|----------|
| 获取治疗计划项目 | GET | `/api/ai/treatment-plan-items/{promptResultId}` | 根据PromptResultId查询关联的诊疗计划项目列表 |
| 批量保存项目 | POST | `/api/ai/treatment-plan-items` | 批量保存诊疗计划项目到数据库 |
| 更新单个项目 | PUT | `/api/ai/treatment-plan-items/{itemId}` | 更新单条诊疗计划项目的内容 |
| 软删除项目 | PUT | `/api/ai/treatment-plan-items/{itemId}/delete` | 软删除单条诊疗计划项目 |
| **新增** | **POST** | **`/api/medicalrecords/todos/from-treatment-plan`** | **从诊疗计划创建待办事项** |
...
```json
{
  "patientId": "990500001204401_1",
  "itemDescription": "开具阿司匹林肠溶片口服医嘱",
  "itemType": "药物",
  "notes": "注意观察有无出血倾向",
  "importanceLevel": "重要",
  "treatmentPlanItemId": 12345
}
```

**章节来源**
- [ai.js:870-1023](file://med_ai_assistant_1.0_bs_vue/src/api/ai.js#L870-L1023)
- [patient.js:628-630](file://med_ai_assistant_1.0_bs_vue/src/api/patient.js#L628-L630)

### 数据库设计

治疗计划表格功能的数据库设计采用了优化的表结构来支持高效查询和数据管理：
...
NUMBER ResultId PK
NUMBER PromptId FK
CLOB originalResultContent
CLOB modifiedResultContent
}
PROMPTS {
NUMBER PromptId PK
NUMBER PatientId FK
...
NUMBER RESULT_ID PK
VARCHAR2 PATIENT_ID
VARCHAR2 ASSESSMENT_TYPE
VARCHAR2 ASSESSMENT_STATUS
NUMBER PRIORITY
TIMESTAMP ASSESSMENT_TIME
}
TREATMENT_PLAN_ITEMS ||--|| PROMPTRESULT : "关联"
PROMPTRESULT ||--|| PROMPTS : "关联"
```

**图表来源**
- [create-treatment-plan-items-table.sql:4-26](file://med_ai_assistant_1.0_bs_backend/sql-scripts/create-treatment-plan-items-table.sql#L4-L26)
...
| IDX_TPI_PATIENT_ID | PATIENT_ID, IS_DELETED, CREATED_AT | 按患者查询 | 15倍 |
...
**重大更新**：本次新增的"加入待办"按钮功能和QC质量控制集成是治疗计划表格功能的重要里程碑，它们不仅提升了用户体验，更重要的是实现了从AI生成内容到实际工作执行的无缝衔接，为整个医疗AI系统的智能化发展奠定了更加坚实的基础。
