...
subgraph "后端数据库优化"
R[TREATMENT_PLAN_ITEMS表]
S[PATIENT_ID冗余字段]
T[单表直查索引]
U[IDX_TPI_PATIENT_ID]
end
subgraph "测试框架"
V[Cypress E2E测试]
W[treatment-plan-todo.cy.js]
X[选中文字测试]
end
A --> R
C --> R
...
subgraph "工具层依赖"
J[treatmentPlanParser.js] --> K[正则表达式]
J --> L[字符串处理]
end
subgraph "API层依赖"
M[ai.js] --> N[Axios HTTP客户端]
M --> O[请求拦截器]
M --> P[响应拦截器]
Q[patient.js] --> R[createTodoFromTreatmentPlan函数]
end
subgraph "状态管理"
S[Vuex Store] --> T[患者状态]
S --> U[Prompt状态]
...
subgraph "测试依赖"
W[Cypress E2E测试] --> X[treatment-plan-todo.cy.js]
Y[选中文字测试] --> Z[AIResults.vue]
end
subgraph "QC服务依赖"
AA[QcAssessmentService.java] --> BB[质控评估服务]
AA --> CC[优先级配置]
end
subgraph "数据库优化依赖"
DD[TreatmentPlanItemRepository] --> EE[PATIENT_ID索引]
DD --> FF[单表直查优化]
end
A --> J
A --> M
...
**重大更新**：本次新增的"加入待办"按钮功能、QC质量控制集成、PATIENT_ID冗余字段优化、URL编码问题修复和选中文字处理功能是治疗计划表格功能的重要里程碑。这些更新不仅提升了用户体验，更重要的是实现了从AI生成内容到实际工作执行的无缝衔接，为整个医疗AI系统的智能化发展奠定了更加坚实的基础。

特别是PATIENT_ID冗余字段的设计和单表直查优化，以及URL编码问题的系统性修复，体现了开发团队对系统性能和稳定性的高度重视。这些优化措施确保了系统在高并发场景下的稳定运行，为医疗AI系统的规模化应用提供了可靠的技术支撑。

**v0.9.003版本的选中文字加入待办功能**和**v0.9.002版本的临床指引重新分析错误处理优化**进一步增强了系统的实用性和稳定性。选中文字处理功能让用户能够更高效地将表格中的内容转化为待办事项，而错误处理优化则提升了系统在异常情况下的用户体验。
