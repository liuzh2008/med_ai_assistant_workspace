...
## 更新摘要
**所做更改**
- 更新了按钮交互系统重设计部分，增加了确认对话框和重新生成功能
- 增强了折叠/展开功能，实现了两级可折叠结构和原生details/summary支持
- 改进了格式约束和排版样式标准化，包括Markdown渲染优化
- 新增了分析过程折叠块和样式定制功能
...
**更新** 本次更新大幅优化了按钮交互系统、折叠/展开功能、格式约束和排版样式，提供了更加用户友好和专业的界面体验。
...
- **两级折叠结构**：支持详细的可折叠内容展示
- **原生HTML5折叠**：使用details/summary实现浏览器原生折叠交互
- **分析过程展示**：专门的分析过程折叠块
- **样式标准化**：统一的排版样式和视觉设计
...
- [DataCollectionAdvice.vue:1-552](file://med_ai_assistant_1.0_bs_vue/src/components/patient/DataCollectionAdvice.vue#L1-L552)
...
**更新** 新增了按钮交互系统重设计，包括确认对话框、重新生成按钮和状态管理优化。
...
**图表来源**
- [DataCollectionAdvice.vue:223-253](file://med_ai_assistant_1.0_bs_vue/src/components/patient/DataCollectionAdvice.vue#L223-L253)
...
- **重新分析按钮**：圆形按钮，带有刷新图标，支持加载状态
- **确认对话框**：用户点击重新分析时弹出确认对话框
- **状态反馈**：生成任务提交后显示成功消息
- **错误处理**：完善的错误状态管理和用户提示
...
B --> C[buildCollapsibleHtml处理]
C --> D[wrapHeadingsInDetails包装]
D --> E[wrapPreamble导言处理]
E --> F[DOMPurify安全过滤]
F --> G[最终渲染的HTML]
```

**图表来源**
- [DataCollectionAdvice.vue:147-155](file://med_ai_assistant_1.0_bs_vue/src/components/patient/DataCollectionAdvice.vue#L147-L155)
...
- **子级折叠块**：基于h3标题的嵌套折叠块  
- **分析过程折叠块**：特殊的导言内容折叠块
- **原生HTML5支持**：使用details/summary实现浏览器原生折叠
...
4. **状态缓存**：避免重复的API调用
5. **原生HTML5折叠**：使用浏览器原生支持，减少JavaScript开销
...
5. **原生HTML5折叠支持**：使用details/summary实现浏览器原生折叠交互
6. **分析过程可视化**：专门的分析过程折叠块展示AI推理过程
7. **按钮交互优化**：确认对话框和重新生成功能提升安全性
8. **样式标准化**：统一的排版样式和视觉设计
...
5. **折叠状态持久化**：记住用户的折叠偏好设置

该功能的成功实现不仅提升了系统的智能化水平，也为后续的功能扩展奠定了坚实的技术基础。新的折叠系统和按钮交互设计显著改善了用户体验，使数据收集建议功能更加专业和易用。