...
- **新增入院记录总结自动生成功能**：定时任务系统新增自动入院记录总结生成功能，减少人工工作量，提高工作效率
...
**新增** 入院记录总结自动生成功能：定时任务系统现已集成自动入院记录总结生成功能，通过智能条件判断和自动化处理，显著减少了医护人员的手工工作量，提高了工作效率。该功能能够自动检测患者的入院记录并生成相应的总结内容，确保后续的AI分析和病历管理流程的顺畅进行。
...
TPG --> ATC
ATC --> AT
MRC["MedicalRecordController<br/>病历记录控制器"]
MRC --> AT
end
```
...
- [TimerPromptGenerator.java:1-2407](file://med_ai_assistant_1.0_bs_backend/src/main/java/com/example/medaiassistant/service/TimerPromptGenerator.java#L1-L2407)
- [MccScreeningService.java](file://med_ai_assistant_1.0_bs_backend/src/main/java/com/example/medaiassistant/service/MccScreeningService.java)
...
- [AIController.java:1-3337](file://med_ai_assistant_1.0_bs_backend/src/main/java/com/example/medaiassistant/controller/AIController.java#L1-L3337)
...
- **新增** 入院记录总结自动生成：通过智能条件判断和自动化处理，自动生成入院记录总结内容。
...
- [MedicalRecordController.java:1-810](file://med_ai_assistant_1.0_bs_backend/src/main/java/com/example/medaiassistant/controller/MedicalRecordController.java#L1-L810)
- [TimerPromptGeneratorController.java:1-100](file://med_ai_assistant_1.0_bs_backend/src/main/java/com/example/medaiassistant/controller/TimerPromptGeneratorController.java#L1-L100)
- [AIView.vue:189-228](file://med_ai_assistant_1.0_bs_vue/src/views/AIView.vue#L189-L228)
- [AIController.java:1945-1972](file://med_ai_assistant_1.0_bs_backend/src/main/java/com/example/medaiassistant/controller/AIController.java#L1945-L1972)
...
- **新增** 入院记录总结自动生成功能，通过TimerPromptGenerator和AIController实现智能条件判断和自动化处理。
...
TPGC["TimerPromptGeneratorController<br/>定时Prompt生成控制器"]
AIView["AIView.vue<br/>前端入院记录总结界面"]
...
MRCT --> DB
TIR --> DB
AIView --> |用户触发| AIController
AIController --> |生成入院记录总结| TPG
MS -.-> MQ
ES -.-> MQ
SP -.-> MQ
TPG -.-> MQ
...
### 入院记录总结生成逻辑
**新增** 定时任务中的入院记录总结生成功能，具有以下特点：

- **智能条件判断**：系统会检查患者是否存在入院记录以及入院记录中现病史内容是否非空
- **自动化处理**：满足条件时自动为患者生成入院记录总结Prompt
- **异常处理**：对生成过程中的异常进行捕获和记录，不影响其他患者的处理
- **状态更新**：生成完成后自动更新相关的告警任务状态

```mermaid
flowchart TD
Start(["定时任务开始"]) --> CheckDiagnosis["检查患者是否有诊断记录"]
CheckDiagnosis --> HasDiagnosis{"有诊断记录？"}
HasDiagnosis --> |否| BasicPrompt["生成基础Prompt<br/>诊断分析、诊疗计划、病情小结"]
HasDiagnosis --> |是| FullProcess["执行完整流程"]
BasicPrompt --> AdmissionCheck["检查入院记录总结生成条件"]
FullProcess --> GenerateDiagnosis["生成诊断分析"]
GenerateDiagnosis --> GenerateTreatment["生成诊疗计划"]
GenerateTreatment --> GenerateSummary["生成病情小结"]
GenerateSummary --> DRGCheck["检查DRG分析生成条件"]
DRGCheck --> DRGGenerated{"DRG分析生成成功？"}
DRGGenerated --> |是| MCCCheck["检查MCC分析生成条件"]
DRGGenerated --> |否| AdmissionCheck
MCCCheck --> MCCGenerated{"MCC分析生成成功？"}
MCCGenerated --> |是| AdmissionCheck
MCCGenerated --> |否| AdmissionCheck
AdmissionCheck --> CheckAdmission["检查入院记录是否存在"]
CheckAdmission --> HasAdmission{"有入院记录？"}
HasAdmission --> |否| SkipAdmission["跳过入院记录总结生成"]
HasAdmission --> |是| ExtractPresentIllness["提取现病史内容"]
ExtractPresentIllness --> PresentIllnessEmpty{"现病史内容非空？"}
PresentIllnessEmpty --> |否| SkipAdmission
PresentIllnessEmpty --> |是| GenerateAdmissionSummary["生成入院记录总结Prompt"]
SkipAdmission --> Complete["完成任务"]
GenerateAdmissionSummary --> Complete
Complete --> End(["定时任务结束"])
```

**图表来源**
- [TimerPromptGenerator.java:658-714](file://med_ai_assistant_1.0_bs_backend/src/main/java/com/example/medaiassistant/service/TimerPromptGenerator.java#L658-L714)
- [TimerPromptGenerator.java:2286-2336](file://med_ai_assistant_1.0_bs_backend/src/main/java/com/example/medaiassistant/service/TimerPromptGenerator.java#L2286-L2336)
...
**新增** 定时任务系统的核心增强功能，通过智能条件判断和自动化处理，显著减少医护人员的手工工作量：

### 功能概述
- **自动检测**：系统自动检测在院患者的入院记录是否存在
- **智能提取**：从入院记录中提取现病史内容，确保内容非空
- **条件判断**：只有当满足"存在入院记录"且"现病史内容非空"两个条件时才生成总结
- **自动化生成**：满足条件时自动为患者生成入院记录总结Prompt
- **异常处理**：对生成过程中的异常进行捕获和记录，不影响其他患者的处理

### 技术实现
- **条件检查**：通过`generateAdmissionRecordSummaryForPatient`方法实现智能条件判断
- **内容提取**：从EMR_CONTENT表中提取入院记录内容，定位"现病史"到"既往史"之间的文本
- **状态更新**：生成完成后自动更新相关的告警任务状态为"已完成"
- **日志记录**：详细的日志记录，便于问题排查和性能监控

### 前端集成
- **用户界面**：AIView.vue提供友好的用户界面，支持手动触发入院记录总结生成
- **智能提示**：系统会自动检测是否存在入院记录总结，如有需要会提示用户生成
- **操作流程**：用户可以通过确认对话框触发入院记录总结的生成

### 业务价值
- **减少人工工作量**：显著减少医护人员的手工录入和总结工作
- **提高工作效率**：通过自动化处理，提高整体工作效率
- **确保数据完整性**：确保每个入院患者都有相应的入院记录总结
- **改善工作流程**：为后续的AI分析和病历管理提供更好的数据基础

```mermaid
sequenceDiagram
participant 用户 as "医护人员"
participant 前端 as "AIView.vue"
participant 后端 as "AIController"
participant 定时任务 as "TimerPromptGenerator"
participant 数据库 as "数据库"
用户->>前端 : "查看患者信息"
前端->>后端 : "检查入院记录总结"
后端->>数据库 : "查询PromptResult"
数据库-->>后端 : "返回查询结果"
后端-->>前端 : "返回检查结果"
alt 无入院记录总结
后端->>前端 : "提示生成入院记录总结"
...
- **新增** AIView.vue 依赖AIController，提供入院记录总结的前端界面。
- **新增** AIController 依赖AlertTaskRepository，提供入院记录总结的状态更新功能。
...
PJ --> MRCT
AV["AIView"] --> AIController
AIController --> AT["AlertTask"]
AIController --> TPG["TimerPromptGenerator"]
```
...
- **新增** 入院记录总结性能：智能条件判断和自动化处理减少了不必要的数据库查询和处理开销。
- **新增** 前端交互性能：AIView.vue中的入院记录总结检查逻辑避免了重复的API调用。
...
- **新增** 入院记录总结异常：检查入院记录存在性、现病史内容提取、生成条件判断。
- **新增** 前端交互异常：检查AIView.vue中的入院记录总结检查逻辑，验证用户提示和确认流程。
- **新增** AI控制器异常：检查入院记录总结的状态更新逻辑，验证告警任务状态变更。
...
**新增** 入院记录总结自动生成功能是本次更新的重要亮点，通过智能条件判断和自动化处理，显著减少了医护人员的手工工作量，提高了工作效率。该功能不仅提升了系统的智能化水平，还为后续的AI分析和病历管理奠定了坚实的数据基础。

建议在生产环境中进一步完善定时任务的配置验证机制，加强定时任务执行的监控和告警功能，优化部门和床位号过滤的性能表现，并持续改进定时任务的配置管理和API接口设计。同时，建议加强对入院记录总结功能的监控和优化，确保其在高并发场景下的稳定性和性能表现。
...
- 性能调优建议
  ...
  - **新增** 优化入院记录总结性能，减少不必要的数据库查询
  - **新增** 优化前端交互性能，避免重复的API调用
...
- **新增** 入院记录总结功能配置
  - 条件检查：支持入院记录存在性检查和现病史内容验证
  - 自动化生成：支持智能条件判断和自动化处理
  - 异常处理：支持生成过程中的异常捕获和记录
  - 状态更新：支持生成完成后自动更新告警任务状态
  - 前端集成：支持用户界面的友好交互和操作提示
- **更新** 批量更新异常处理配置
