...
**更新** 本次更新反映了数据库连接配置的重大变更：IP地址从100.66.1.3迁移到100.66.1.4，Oracle SID从FREE更新为XE，确保测试环境配置的准确性和一致性。
...
if telnet 100.66.1.4 1521 2>/dev/null; then
    ...
    # 测试数据库连接
    docker exec med-ai-main java -cp /app/lib/ojdbc11.jar:/app/classes \
...
**更新** 本次更新增强了系统的配置管理和监控能力，特别是CORS配置的外部化和改进的健康检查机制，以及准确的数据库连接配置。
