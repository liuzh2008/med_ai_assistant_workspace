...
**章节来源**
- [application.properties:1-193](file://med_ai_assistant_1.0_bs_backend/deploy/main-linux-testServer/config/application.properties#L1-L193)
- [docker-compose-main.yml:1-110](file://med_ai_assistant_1.0_bs_backend/deploy/main-linux-testServer/docker-compose-main.yml#L1-L110)
...
subgraph "数据层"
H[Oracle数据库] --> I[主服务器数据库]
J[MySQL数据库] --> K[执行服务器数据库]
L[Redis缓存] --> M[会话存储]
end
subgraph "监控层"
N[健康检查] --> O[日志监控]
...
| Docker配置 | docker-compose.yml | 容器编排 | services、volumes |
