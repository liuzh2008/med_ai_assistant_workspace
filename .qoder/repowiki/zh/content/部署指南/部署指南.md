...
## 更新摘要
**变更内容**
- 新增GitHub Actions前端发布工作流：完善了release-frontend.yml工作流，实现前端自动化构建和发布，包含E2E测试和截图上传功能
...
- 修复接口路径问题：修正后端自动部署接口路径，避免URL重复问题
- 增强自动部署功能：前后端自动部署脚本均支持版本号+文件大小双校验，提供完整的错误恢复机制
- **新增** 完善前端部署指南：包括nginx反向代理配置、/api请求转发到后端服务器、VUE_APP_EXECUTION_SERVER_URL环境变量配置等
...
**更新** 本版本重点反映了CI/CD自动化部署的重大改进：新增GitHub Actions前端发布工作流，实现前端自动化构建和发布；完善了前后端一体化的CI/CD流程，通过medai-builds私有仓库集中管理构建产物，支持执行服务器下载和主服务器部署。采用本地JAR+Docker复制模式的构建流程优化显著提升了构建效率和稳定性。**重要更新** v0.4.068版本修复的接口路径问题确保了前后端配置的正确配合，避免了URL重复的问题。**维护性更新** 包含子模块同步和文档基础设施维护，确保部署流程与最新的文档版本保持一致。
...
- **API代理配置**
  ...
  - proxy_pass http://10.120.11.43:8081/api/
...
- **作用**：设置后端API的基础URL
- **默认值**：http://10.120.11.43:8081/api
...
#### 主API配置（request.js）
- **axios实例**：service
- **baseURL**：process.env.VUE_APP_API_BASE_URL || 'http://10.120.11.43:8081/api'
- **超时设置**：30秒
...
- **API请求失败**
  - **新增** 检查nginx.conf中的/api代理配置，确认proxy_pass地址正确。
...
- **前端部署验证**
  ...
  - **新增** 验证nginx.conf配置，检查/api代理转发是否正常。
