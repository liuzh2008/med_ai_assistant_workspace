...
## 更新摘要
**变更内容**
- 新增前端自动部署功能：UpdateView.vue组件移除手动部署步骤，新增自动部署前端功能
- 新增POST /api/deploy/auto-deploy-frontend接口文档：支持一键自动部署前端
- 新增部署脚本和版本管理流程：创建auto-deploy-frontend.sh和restore-frontend.sh脚本
- 更新构建流程优化：采用本地JAR+Docker复制模式，提升构建效率和稳定性
- 新增后端自动部署功能：支持后端服务的自动部署和版本管理
- 增强CI/CD部署流程：完善GitHub Actions工作流和版本号管理
- **接口路径修正**：后端自动部署接口路径从重复的'/api/deploy/auto-deploy-backend'修正为正确的'/deploy/auto-deploy-backend'，避免与VUE_APP_API_BASE_URL中的'/api'基础URL重复，确保正确的URL构造'/api/deploy/auto-deploy-backend'
...
**更新** 本版本重点反映了前端自动部署功能的重大改进：UpdateView.vue组件移除了手动部署步骤，新增了自动部署前端功能，通过POST /api/deploy/auto-deploy-frontend接口实现一键部署，大幅简化了部署流程。同时新增了后端自动部署功能，提供完整的版本管理和错误恢复机制。**重要更新** v0.4.068版本修复了后端自动部署接口路径问题，确保与前端VUE_APP_API_BASE_URL配置正确配合。

## 项目结构
项目采用前后端分离的多仓库布局，并通过子模块或独立仓库形式管理。根目录下提供后端与前端的Docker与Compose配置，以及针对不同场景（主服务-Oracle、执行服务器-Linux、生产前端等）的专用编排文件。
...
## 架构总览
下图展示生产环境典型拓扑：前端通过Nginx反向代理对外提供服务；后端主服务与执行服务器分别承载业务与执行任务；MySQL与Redis作为数据与缓存支撑；Oracle数据库对接HIS/LIS系统。
...
## 结论
本指南基于仓库现有配置，提供了从多阶段构建、容器编排到Oracle集成与生产部署的完整路径。**更新** 前端自动部署功能的引入大幅简化了部署流程，通过UpdateView.vue组件和POST /api/deploy/auto-deploy-frontend接口实现了真正的"一键部署"。采用本地JAR+Docker复制模式的构建流程优化显著提升了构建效率和稳定性，建议在生产环境中结合负载均衡、高可用部署与安全加固进一步完善整体方案。

**新增** 后端自动部署功能提供了完整的版本管理和错误恢复机制，支持后端服务的自动化部署和维护。CI/CD部署流程的优化确保了构建的一致性和可靠性。**重要更新** v0.4.068版本修复的接口路径问题确保了前后端配置的正确配合，避免了URL重复的问题。这些改进共同构成了MedAiAssistant 1.0 BS的现代化部署解决方案。
...
- **前端部署流程优化**
  - **新增** 自动部署脚本支持版本检查，避免重复部署。
  - **新增** 完整的错误恢复机制，部署失败时自动回滚到备份版本。
  - **新增** 详细的日志记录，便于问题排查和审计。
- **后端部署流程优化**
  - **新增** 自动部署脚本支持版本号+文件大小双校验，避免重复部署。
  - **新增** 完整的错误恢复机制，部署失败时自动回滚到备份版本。
  - **新增** 600秒超时设置，确保长时间部署任务的稳定性。
