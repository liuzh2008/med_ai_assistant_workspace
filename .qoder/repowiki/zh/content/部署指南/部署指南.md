...
- [nginx-proxy.conf](file://med_ai_assistant_1.0_bs_vue/nginx-proxy.conf)
- [nginx.conf（部署）](file://med_ai_assistant_1.0_bs_vue/deploy/med_ai_assistant_1.0_bs_vue/nginx.conf)
- [nginx.main.conf（部署）](file://med_ai_assistant_1.0_bs_vue/deploy/med_ai_assistant_1.0_bs_vue/nginx.main.conf)
- [vue.config.js](file://med_ai_assistant_1.0_bs_vue/vue.config.js)
- [cypress.env.json.example](file://med_ai_assistant_1.0_bs_vue/cypress.env.json.example)
...
- [nginx.conf（测试）](file://med_ai_assistant_1.0_bs_vue/deploy/med_ai_assistant_1.0_bs_vue_test/nginx.conf)
- [nginx-proxy.conf（测试）](file://med_ai_assistant_1.0_bs_vue/deploy/med_ai_assistant_1.0_bs_vue_test/nginx-proxy.conf)
</cite>

## 更新摘要
**变更内容**
- 基础设施优化：移除硬编码IP地址，采用相对路径和环境变量配置
- 反向代理优化：改进nginx配置，支持相对路径代理和域名配置
- 环境可移植性：增强环境变量配置，支持不同部署环境的灵活切换
- 部署架构改进：优化部署流程，提升环境适应性和可维护性
...
**更新** 本版本重点反映了基础设施优化的重大改进：移除了硬编码IP地址，采用相对路径和环境变量配置；改进了nginx反向代理配置，支持相对路径代理和域名配置；增强了环境可移植性，支持不同部署环境的灵活切换。这些改进显著提升了部署架构的灵活性和可维护性。
...
C --> C8["nginx.conf反向代理配置"]
C --> C9["vue.config.js开发服务器代理配置"]
```
...
- 前端服务（Nginx）
  ...
  - **新增** nginx反向代理配置：支持/api请求转发到后端服务器，配置CORS跨域支持。
  - **新增** VUE_APP_EXECUTION_SERVER_URL环境变量：支持执行服务器地址配置。
- 执行服务器（Linux）
...
MAIN --> FRONTEND_AUTO_DEPLOY["前端自动部署接口<br/>/api/deploy/auto-deploy-frontend"]
EXEC["后端执行服务器<br/>端口 8082"]
EXEC --> BUILD_FILES["构建产物接口<br/>/api/build-files/*"]
EXEC --> EXEC_API["执行服务器API<br/>/api/execute/*"]
end
subgraph "数据层"
MYSQL["MySQL 8.0"]
...
- **生产反向代理**
  ...
  - **新增** 完整的nginx.conf配置，支持/api请求转发到后端服务器，配置CORS跨域支持。
  - **新增** 支持Vue路由的单页应用配置，所有请求返回index.html。
  - **新增** 支持相对路径代理，移除硬编码IP地址，增强环境可移植性。
...
Nginx->>Backend : 转发/api请求
Backend-->>Nginx : 返回API响应
Nginx-->>Client : 返回API响应
```

**图示来源**
- [docker-compose.prod.yml（前端-生产）:27-48](file://med_ai_assistant_1.0_bs_vue/deploy/med_ai_assistant_1.0_bs_vue/docker-compose.prod.yml#L27-L48)
- [Dockerfile（前端）:1-30](file://med_ai_assistant_1.0_bs_vue/Dockerfile#L1-L30)
- [nginx.conf（部署）:45-74](file://med_ai_assistant_1.0_bs_vue/deploy/med_ai_assistant_1.0_bs_vue/nginx.conf#L45-L74)
...
- [nginx.conf（部署）:1-97](file://med_ai_assistant_1.0_bs_vue/deploy/med_ai_assistant_1.0_bs_vue/nginx.conf#L1-L97)
...
**图示来源**
- [release-frontend.yml:1-109](file://.github/workflows/release-frontend.yml#L1-L109)
...
**新增** 完善的前端部署配置指南，包括nginx反向代理、环境变量配置等

### nginx反向代理配置

#### 生产环境nginx.conf配置
- **基础配置**
  - 监听80端口，server_name设置为localhost
  - 启用gzip压缩，配置静态资源缓存策略
  - 支持Vue单页应用路由，所有请求返回index.html
- **API代理配置**
  - location /api/ 区块将/api请求转发到后端服务
  - **更新** 使用相对路径代理：proxy_pass http://med-ai-main-server:8081/api/
  - 配置CORS跨域支持，允许所有方法和头部
  - 设置超时时间为300秒，支持长连接
- **安全头设置**
...
#### 开发环境vue.config.js配置
- **开发服务器代理**
  - 代理/api请求到http://localhost:8081
  - 设置请求超时时间为310秒，支持LLM流式响应
  - 保持/api前缀，后端路径包含/api
- **构建配置**
  - publicPath设置为'/'，适配反向代理
  - 启用raw-loader处理.md文件
...
- **作用**：设置后端API的基础URL
- **默认值**：http://localhost:8081/api
- **用途**：主API axios实例的基础URL
- **配置方式**：process.env.VUE_APP_API_BASE_URL

##### VUE_APP_EXECUTION_SERVER_URL
- **作用**：设置执行服务器的地址
- **默认值**：http://100.66.1.3:8082
- **用途**：执行服务器axios实例的基础URL
- **配置方式**：process.env.VUE_APP_EXECUTION_SERVER_URL

##### VUE_APP_DECRYPTION_SERVER_URL
- **作用**：设置解密服务器的地址
- **默认值**：http://localhost:8082
- **用途**：解密服务器axios实例的基础URL
- **配置方式**：process.env.VUE_APP_DECRYPTION_SERVER_URL
...
- **请求拦截器**：添加Authorization头，包含JWT token

#### 执行服务器API配置（request.js）
- **axios实例**：executionService
- **baseURL**：process.env.VUE_APP_EXECUTION_SERVER_URL || 'http://100.66.1.3:8082'
- **超时设置**：30秒
- **响应类型**：JSON
- **用途**：AI模型执行、任务处理等

#### 解密服务器API配置（request.js）
- **axios实例**：decryptionService
...
#### 生产环境部署脚本
- **deploy-linux.sh**：Linux环境前端部署脚本
- **功能**：检查环境、下载构建包、部署到Nginx
- **依赖**：nginx.conf、docker-compose.prod.yml

#### 开发环境配置
- **vue.config.js**：开发服务器代理配置
- **cypress.env.json.example**：测试环境配置示例
- **package.json**：脚本命令定义

```mermaid
flowchart TD
A["前端部署流程"] --> B["nginx.conf配置"]
B --> C["API代理设置"]
C --> D["CORS配置"]
D --> E["安全头设置"]
E --> F["开发服务器代理"]
...
**章节来源**
- [nginx.conf（部署）:1-97](file://med_ai_assistant_1.0_bs_vue/deploy/med_ai_assistant_1.0_bs_vue/nginx.conf#L1-L97)
- [nginx.main.conf（部署）:1-55](file://med_ai_assistant_1.0_bs_vue/deploy/med_ai_assistant_1.0_bs_vue/nginx.main.conf#L1-L55)
- [vue.config.js:1-26](file://med_ai_assistant_1.0_bs_vue/vue.config.js#L1-L26)
- [cypress.env.json.example:1-6](file://med_ai_assistant_1.0_bs_vue/cypress.env.json.example#L1-L6)
...
- **前端部署性能**
  ...
  - **新增** nginx反向代理配置优化，支持Gzip压缩和静态资源缓存。
  - **新增** 环境变量配置，支持灵活的服务地址设置。
- **后端部署性能**
...
- **前端无法访问**
  ...
  - **新增** 检查nginx.conf中的/api代理配置是否正确，确认使用相对路径而非硬编码IP。
  - **新增** 验证VUE_APP_API_BASE_URL环境变量设置。
- **构建失败**
  - 确认本地JAR文件存在且可访问；检查Dockerfile中的COPY路径正确性。
- **前端自动部署失败**
  - **新增** 检查执行服务器的构建文件接口是否可用；确认主服务器的部署接口权限；查看自动部署脚本的详细输出日志。
  - **新增** 验证nginx.conf中的/api路径转发，确认使用相对路径代理。
...
- **API请求失败**
  - **新增** 检查nginx.conf中的/api代理配置，确认proxy_pass地址使用相对路径而非硬编码IP。
  - **新增** 验证VUE_APP_API_BASE_URL环境变量，确保后端服务地址正确。
  - **新增** 检查CORS配置，确认Access-Control-Allow-Origin设置。
...
- **注意事项**
  ...
  - **新增** Windows环境下可使用nginx-proxy.conf进行开发环境代理配置。

**章节来源**
- [mvn.bat:1-5](file://mvn.bat#L1-L5)
- [npm.bat:1-3](file://npm.bat#L1-L3)
- [nginx-proxy.conf:1-87](file://med_ai_assistant_1.0_bs_vue/nginx-proxy.conf#L1-L87)
...
- **前端（生产）**
  ...
  - **新增** 配置nginx.conf反向代理，支持/api请求转发。
  - **新增** 使用相对路径代理，移除硬编码IP地址。
- **自动部署脚本**
  ...
  - **新增** 配置构建产物下载目录和权限。
  - **新增** 配置环境变量VUE_APP_API_BASE_URL、VUE_APP_EXECUTION_SERVER_URL等。
...
- **安全**
  ...
  - **新增** nginx反向代理配置包含安全头设置，提升安全性。
  - **新增** CORS配置支持跨域访问，确保前后端通信正常。
  - **新增** 移除硬编码IP地址，采用相对路径代理，增强环境可移植性。
- **健康检查**
  - 合理设置探测参数，避免频繁重启与误判。
...
- [nginx.conf（部署）:11-15](file://med_ai_assistant_1.0_bs_vue/deploy/med_ai_assistant_1.0_bs_vue/nginx.conf#L11-L15)
...
- **前端部署验证**
  ...
  - **新增** 验证nginx.conf配置，检查/api代理转发是否正常，确认使用相对路径。
  - **新增** 验证环境变量配置，确认VUE_APP_API_BASE_URL等设置正确。
- **后端部署验证**
  ...
  - **新增** 验证API请求转发，检查CORS配置和超时设置。
  - **新增** 验证相对路径代理配置，确保移除了硬编码IP地址。
...
**新增** 主服务器从执行服务器下载构建产物的实现方案

- **接口设计**
  - 主服务器通过代理转发执行服务器的构建产物下载接口
  - 支持查询最新版本、触发下载、查询本地文件状态
- **实现细节**
  - BuildDownloadController：实现构建产物下载的后端控制器
  - application.properties：新增构建产物下载目录配置
  - docker-compose：新增volume挂载配置，实现宿主机目录映射
- **部署步骤**
  - 在宿主机创建下载目录并设置权限
  - 重启主服务器容器使volume挂载生效
  - 验证挂载是否正确，检查目录权限
...
- [BuildDownloadController.java](file://med_ai_assistant_1.0_bs_backend/src/main/java/com/example/medaiassistant/controller/BuildDownloadController.java)
- [application.properties](file://med_ai_assistant_1.0_bs_backend/src/main/resources/application.properties)
- [主服务器从执行服务器下载构建产物实现方案.md:1-242](file://med_ai_assistant_1.0_bs_backend/doc/布署/自动化部署/主服务器从执行服务器下载构建产物实现方案.md#L1-L242)