...
- [Oracle数据库PGA内存超限错误修复-2026年03月14日.md](file://med_ai_assistant_1.0_bs_backend/doc/更新日志/2026-03-14.md)
...
- [新测试服务器部署与前后端Docker容器化运行.md](file://med_ai_assistant_1.0_bs_backend/doc/测试/测试服务器/主服务器/新测试服务器部署与前后端Docker容器化运行.md)
- [sync_via_dblink.sql](file://med_ai_assistant_1.0_bs_backend/deploy/main-linux-testServer/sql/sync_via_dblink.sql)
- [full_sync.sql](file://med_ai_assistant_1.0_bs_backend/deploy/main-linux-testServer/sql/full_sync.sql)
- [verify.sql](file://med_ai_assistant_1.0_bs_backend/deploy/main-linux-testServer/sql/verify.sql)
...
- 新增测试服务器部署文档中的数据库同步最佳实践
- 新增Oracle版本兼容性问题的完整解决方案
...
G["部署配置目录<br/>deploy/main-linux-testServer/config"] --> H["application.properties"]
I["Java配置类<br/>OracleDatabaseProperties.java"] --> J["Oracle数据库配置管理"]
K["主数据库配置<br/>DatabaseConfig.java"] --> L["事务管理器配置"]
M["执行服务器配置<br/>ExecutionServerDataSourceConfig.java"] --> N["独立事务管理器"]
O["项目依赖<br/>pom.xml"] --> P["Oracle JDBC驱动 ojdbc11"]
Q["初始化脚本<br/>init.sql"] --> R["数据库Schema与基础表"]
S["版本更新日志<br/>更新小结.md"] --> T["SQL外部化改造"]
U["服务层<br/>EncryptedDataTempService.java"] --> V["Hibernate缓存管理策略"]
W["Repository层<br/>ExecutionServerEncryptedDataTempRepository.java"] --> X["JPQL更新方法"]
Y["错误分析文档<br/>ORA-01407约束违规错误分析与解决方案.md"] --> Z["缓存一致性保证"]
AA["Oracle 12c R2配置<br/>主服务器技术栈与配置规范.md"] --> BB["HikariCP优化参数"]
CC["SSH连接配置<br/>execution-windows/README.md"] --> DD["端口16601配置"]
EE["数据库同步<br/>新测试服务器部署与前后端Docker容器化运行.md"] --> FF["Database Link + CTAS方案"]
GG["同步脚本<br/>sync_via_dblink.sql"] --> HH["Database Link创建"]
II["同步脚本<br/>full_sync.sql"] --> JJ["CTAS全量同步"]
KK["同步脚本<br/>verify.sql"] --> LL["关键列验证"]
MM["同步脚本<br/>gen_ddl.sql"] --> NN["DDL生成"]
OO["连接稳定性<br/>数据库连接稳定性修复说明.md"] --> PP["网络超时参数优化"]
QQ["PGA内存问题<br/>Oracle数据库PGA内存超限错误修复-2026年03月14日.md"] --> RR["内存限制与优化"]
```
...
- [ExecutionServerDataSourceConfig.java:1-162](file://med_ai_assistant_1.0_bs_backend/src/main/java/com/example/medaiassistant/config/ExecutionServerDataSourceConfig.java#L1-L162)
...
- [ORA-01407约束违规错误分析与解决方案.md:1-136](file://med_ai_assistant_1.0_bs_backend/doc/其他/ORA-01407约束违规错误分析与解决方案.md#L1-L136)
...
- [新测试服务器部署与前后端Docker容器化运行.md:1-262](file://med_ai_assistant_1.0_bs_backend/doc/测试/测试服务器/主服务器/新测试服务器部署与前后端Docker容器化运行.md#L1-L262)
- [sync_via_dblink.sql:1-20](file://med_ai_assistant_1.0_bs_backend/deploy/main-linux-testServer/sql/sync_via_dblink.sql#L1-L20)
- [full_sync.sql:1-58](file://med_ai_assistant_1.0_bs_backend/deploy/main-linux-testServer/sql/full_sync.sql#L1-L58)
- [verify.sql:1-4](file://med_ai_assistant_1.0_bs_backend/deploy/main-linux-testServer/sql/verify.sql#L1-L4)
...
- [database连接稳定性修复说明.md:1-35](file://med_ai_assistant_1.0_bs_backend/doc/问题修复/数据库连接稳定性修复说明.md#L1-L35)
...
- [application.properties（main-linux-testServer）:88-95](file://med_ai_assistant_1.0_bs_backend/deploy/main-linux-testServer/config/application.properties#L88-L95)
- [pom.xml:98-124](file://med_ai_assistant_1.0_bs_backend/pom.xml#L98-L124)
- [init.sql:1-119](file://med_ai_assistant_1.0_bs_backend/init.sql#L1-L119)
...
- [主服务器技术栈与配置规范.md:172-210](file://med_ai_assistant_1.0_bs_backend/doc/其他/主服务器技术栈与配置规范.md#L172-L210)
- [README.md（execution-windows）:60-106](file://med_ai_assistant_1.0_bs_backend/deploy/execution-windows/README.md#L60-L106)
- [新测试服务器部署与前后端Docker容器化运行.md:36-92](file://med_ai_assistant_1.0_bs_backend/doc/测试/测试服务器/主服务器/新测试服务器部署与前后端Docker容器化运行.md#L36-L92)
...
ENDPOINT9["PGA内存管理<br/>Free版本2GB限制"]
ENDPOINT10["BG00后台进程<br/>1.6GB内存占用"]
ENDPOINT11["同步源数据库<br/>Oracle 23ai"]
ENDPOINT12["同步目标数据库<br/>Oracle 21c XE"]
ENDPOINT13["Database Link<br/>remote_xe"]
ENDPOINT14["CTAS同步<br/>DROP + CREATE TABLE AS SELECT"]
ENDPOINT15["DDL生成<br/>DBMS_METADATA.GET_DDL"]
ENDPOINT16["关键列验证<br/>INHOSPITAL_EXISTS"]
end
subgraph "数据访问层"
JPA["JPA/Hibernate"]
HIK["HikariCP 连接池"]
...
ENDPOINT17["Oracle 12c R2优化<br/>HikariCP参数"]
ENDPOINT18["同步验证<br/>COUNT(*)比对"]
end
subgraph "数据库层"
ORA["Oracle 数据库"]
ORA2["Oracle 12c R2"]
...
ENDPOINT19["同步源表<br/>user_tables@dev_db"]
ENDPOINT20["同步目标表<br/>本地表结构"]
ENDPOINT21["兼容性处理<br/>语法清理与类型转换"]
ENDPOINT22["内存限制<br/>2GB PGA限制"]
ENDPOINT23["后台进程<br/>BG00 1.6GB占用"]
end
APP --> CFG
APP --> ORACLE_CFG
APP --> DB_CONFIG
...
CACHE --> ENDPOINT7
CACHE --> ENDPOINT6
ENDPOINT3 --> ENDPOINT13
ENDPOINT3 --> ENDPOINT14
ENDPOINT3 --> ENDPOINT15
ENDPOINT3 --> ENDPOINT16
ENDPOINT13 --> ENDPOINT19
ENDPOINT14 --> ENDPOINT20
ENDPOINT15 --> ENDPOINT21
ENDPOINT16 --> ENDPOINT21
ENDPOINT18 --> ENDPOINT19
ENDPOINT18 --> ENDPOINT20
ENDPOINT9 --> ENDPOINT22
ENDPOINT9 --> ENDPOINT23
```
...
- [application-oracle.properties:1-11](file://med_ai_assistant_1.0_bs_backend/src/main/resources/application-oracle.properties#L1-L11)
- [application.properties（main-linux-testServer）:112-117](file://med_ai_assistant_1.0_bs_backend/deploy/main-linux-testServer/config/application.properties#L112-L117)
- [主服务器技术栈与配置规范.md:172-210](file://med_ai_assistant_1.0_bs_backend/doc/其他/主服务器技术栈与配置规范.md#L172-L210)
...
- [application.properties（main-linux-testServer）:24-44](file://med_ai_assistant_1.0_bs_backend/deploy/main-linux-testServer/config/application.properties#L24-L44)
- [主服务器技术栈与配置规范.md:172-210](file://med_ai_assistant_1.0_bs_backend/doc/其他/主服务器技术栈与配置规范.md#L172-L210)
...
#### **兼容性处理清单**
- **STORAGE子句**：删除
- **LOB STORE AS BASICFILE**：删除
- **TABLESPACE子句**：删除
- **COMPUTE STATISTICS**：删除
- **COMPRESS NO残留**：正则清理`) NO ;` → `);`
- **BOOLEAN类型**：改为`NUMBER(1,0)`
- **GENERATED BY DEFAULT AS IDENTITY**：简化为`...AS IDENTITY`
- **SEQUENCE.NEXTVAL默认值**：删除
- **中文列名乱码**：从数据库直接查询正确列名，手动重构
...
**章节来源**
- [ORA-01407约束违规错误分析与解决方案.md:94-136](file://med_ai_assistant_1.0_bs_backend/doc/其他/ORA-01407约束违规错误分析与解决方案.md#L94-L136)
...
SYNC --> CTAS["CTAS同步方案"]
SYNC --> DDL_GEN["DDL生成脚本"]
SYNC --> VERIFY["验证脚本"]
SSH["SSH连接配置"] --> ORACLE_PORT["端口16601"]
ORA_12C["Oracle 12c R2优化"] --> HIKARI["HikariCP参数"]
PGA["PGA内存管理"] --> FREE_LIMIT["2GB内存限制"]
FREE_LIMIT --> BG00["BG00后台进程 1.6GB"]
```
...
- **PGA内存管理**
  ...
  - **新增**：通过ALTER SYSTEM SET PGA_AGGREGATE_LIMIT = 2048M解决内存超限问题
- **事务管理优化**
...
- **Oracle版本兼容性问题**
  ...
  - 参考
    - [新测试服务器部署与前后端Docker容器化运行.md:36-92](file://med_ai_assistant_1.0_bs_backend/doc/测试/测试服务器/主服务器/新测试服务器部署与前后端Docker容器化运行.md#L36-L92)
- **PGA内存超限问题**
  - 现象：ORA-04036错误，PGA内存使用超过限制
  - 排查：确认pga_aggregate_limit设置为2G，重启Oracle容器
  - 参考
    - [Oracle数据库PGA内存超限错误修复-2026年03月14日.md:1-173](file://med_ai_assistant_1.0_bs_backend/doc/问题修复/Oracle数据库PGA内存超限错误修复-2026年03月14日.md#L1-L173)
- **数据库同步失败**
  - 现象：CTAS同步过程中出现表结构不匹配
  - 排查：确认使用DDL生成脚本，检查兼容性处理清单
  - 参考
    - [gen_ddl.sql:1-21](file://med_ai_assistant_1.0_bs_backend/deploy/main-linux-testServer/sql/gen_ddl.sql#L1-L21)
    - [新测试服务器部署与前后端Docker容器化运行.md:61-73](file://med_ai_assistant_1.0_bs_backend/doc/测试/测试服务器/主服务器/新测试服务器部署与前后端Docker容器化运行.md#L61-L73)
...
- 测试服务器部署文档中的数据库同步最佳实践，包括DDL生成和验证
- PGA内存超限问题的完整解决方案，包括监控和预防措施
...
- **配置清单（按类别）**
  ...
  - **Oracle PGA内存超限问题**
    - 问题描述与影响
      - [Oracle数据库PGA内存超限错误修复-2026年03月14日.md:1-173](file://med_ai_assistant_1.0_bs_backend/doc/问题修复/Oracle数据库PGA内存超限错误修复-2026年03月14日.md#L1-L173)
    - 解决方案与预防措施
      - [Oracle数据库PGA内存超限错误修复-2026年03月14日.md:64-98](file://med_ai_assistant_1.0_bs_backend/doc/问题修复/Oracle数据库PGA内存超限错误修复-2026年03月14日.md#L64-L98)