...
- [application-prompt.properties（main-linux-oracle）](file://med_ai_assistant_1.0_bs_backend/deploy/main-linux-oracle/config/application-prompt.properties)
- [application-prompt.properties（main-linux-testServer）](file://med_ai_assistant_1.0_bs_backend/deploy/main-linux-testServer/config/application-prompt.properties)
...
- [PromptServiceConfigTest.java](file://med_ai_assistant_1.0_bs_backend/src/test/java/com/example/medaiassistant/config/PromptServiceConfigTest.java)
- [PromptServiceConfigSimpleTest.java](file://med_ai_assistant_1.0_bs_backend/src/test/java/com/example/medaiassistant/config/PromptServiceConfigSimpleTest.java)
- [PromptServiceConfigIntegrationTest.java](file://med_ai_assistant_1.0_bs_backend/src/test/java/com/example/medaiassistant/config/PromptServiceConfigIntegrationTest.java)
...
## 更新摘要
**所做更改**
- 新增R-003性能优化配置章节，详细说明application-prompt.properties的优化内容
- 更新Prompt服务配置章节，反映提交页面大小、轮询页面大小、批处理大小的优化
- 新增PromptServiceConfig配置类的详细说明，包括R-003优化后的参数
- 更新配置验证与测试章节，反映新增的配置优化功能
- 新增性能优化最佳实践与故障排查指导
...
**更新** 本次更新重点反映了R-003性能优化配置，包括application-prompt.properties的提交页面大小、轮询页面大小、批处理大小、线程管理和超时设置的优化，进一步完善了项目的配置管理体系。
...
PROMPT_PROP["config/application-prompt.properties<br/>R-003优化"]
PROMPT_CLASS["PromptServiceConfig.java<br/>配置类映射"]
...
PROMPT_CLASS --> ORA_PROD
PROMPT_CLASS --> TEST_PROD
PROMPT_CLASS --> DEV_PROP
...
- [PromptServiceConfig.java:1-308](file://med_ai_assistant_1.0_bs_backend/src/main/java/com/example/medaiassistant/config/PromptServiceConfig.java#L1-L308)
...
- **Prompt服务配置（R-003优化）**：application-prompt.properties 控制提交、轮询与监控的频率、重试与超时等行为。**新增** PromptServiceConfig配置类提供完整的配置类映射，支持提交页面大小从10提升到20、轮询页面大小从20提升到50、轮询批次大小从50提升到100的性能优化。
...
**更新** 新增R-003性能优化配置和PromptServiceConfig配置类，通过系统化的配置管理和性能优化，进一步完善了项目的配置管理体系。新增MonitoringProperties配置类提供更精细的监控参数管理。
...
PROMPT["application-prompt.properties<br/>R-003优化"]
PROMPT_CLASS["PromptServiceConfig.java<br/>配置类映射"]
...
PROMPT_CLASS --> PROMPT
PROMPT_CLASS --> ENV_ORA
PROMPT_CLASS --> ENV_TEST
PROMPT_CLASS --> DEV_ENV
...
- **R-003性能优化**：
  - **提交页面大小优化**：从10提升到20，提高吞吐量
  - **轮询页面大小优化**：从20提升到50，提高查询效率  
  - **轮询批次大小优化**：从50提升到100，确保轮询吞吐量 >= 提交吞吐量的2倍
- **参数要点**：提交与轮询的间隔、页面/批次大小、最大重试次数与重试间隔、连接/读取超时等。
- **配置类映射**：通过PromptServiceConfig配置类提供完整的参数映射，支持配置验证和摘要功能。
- 使用建议
  - 根据下游服务吞吐与稳定性调整提交/轮询间隔与重试策略。
  - 监控模块的阈值应与业务 SLA 对齐，避免过早触发告警或漏报异常。
  - **性能优化**：合理设置页面大小和批次大小，平衡吞吐量与内存使用。

**更新** 新增R-003性能优化配置，详细说明提交页面大小、轮询页面大小、批处理大小的优化内容，以及PromptServiceConfig配置类的映射功能。
...
- 关键参数
  ...
  - **Prompt服务**：通过PromptServiceConfig配置类管理R-003性能优化后的配置参数。
- 环境变量注入：多处参数通过 ${VAR:default} 形式注入，便于在不同环境灵活替换。

**更新** 生产环境语音识别配置已修复，baseUrl从http://10.120.8.85:3001改为https://dashscope.aliyuncs.com/compatible-mode/v1，apiKey使用正确的阿里云DashScope API Key。同时，JpaTransactionManager日志级别已统一设置为WARN级别。定时任务配置新增enabled参数，支持定时任务的启用/禁用控制。Prompt服务配置通过PromptServiceConfig配置类实现R-003性能优化。
...
#### 优化概述
R-003性能优化是对Prompt服务配置的重大性能改进，主要针对提交、轮询和监控服务的参数进行了系统性优化，以提高整体吞吐量和响应效率。

#### 提交服务优化（SubmissionConfig）
- **提交页面大小优化**：从10条记录提升到20条记录，提高单次处理吞吐量
- **线程管理优化**：保持maxThreads=5，确保合理的并发控制
- **重试策略**：maxRetries=3，retryInterval=1000ms，平衡可靠性与性能
- **超时设置**：connectTimeout=300000ms，readTimeout=300000ms，支持长时间处理

#### 轮询服务优化（PollingConfig）
- **轮询页面大小优化**：从20条记录提升到50条记录，提高查询效率
- **批处理大小优化**：从50条记录提升到100条记录，确保轮询吞吐量 >= 提交吞吐量的2倍
- **重试策略**：maxRetries=5，retryInterval=2000ms，支持更长的轮询周期
- **超时设置**：timeout=30000ms，支持快速响应和超时处理

#### 监控服务优化（MonitoringConfig）
- **健康检查超时**：health-check-timeout=10000ms，支持快速健康检查
- **指标收集间隔**：metrics-interval=30000ms，平衡监控频率与性能开销
- **告警阈值**：error-rate-threshold=5.0%，response-time-threshold=10000ms，queue-length-threshold=100
- **监控间隔**：interval=60000ms，确保稳定的监控频率

#### 配置类映射（PromptServiceConfig）
- **完整的参数映射**：通过@ConfigurationProperties(prefix = "prompt")实现完整的配置类映射
- **配置验证**：提供isValid()方法验证配置的有效性
- **配置摘要**：提供详细的配置摘要方法，便于调试和监控
- **默认值管理**：Java类中的默认值与配置文件中的值相互补充

#### 性能优化原理
- **吞吐量提升**：通过增加页面大小和批处理大小，显著提高数据处理效率
- **资源利用率优化**：合理的线程管理和超时设置，避免资源浪费
- **系统稳定性保障**：完善的重试机制和超时处理，确保系统稳定运行
- **监控可视化**：详细的配置摘要和监控参数，便于性能分析和问题排查

#### 配置验证与测试
- **单元测试**：完整的配置类单元测试，验证默认值和属性绑定
- **集成测试**：Spring Boot集成测试，验证配置在实际环境中的表现
- **配置摘要测试**：验证配置摘要方法的正确性和可读性
- **默认值测试**：验证Java类中的默认值与配置文件中的值的一致性

**新增** 详细介绍R-003性能优化配置的完整实现，包括具体的优化参数、配置类映射、性能优化原理和测试验证。

**章节来源**
- [application-prompt.properties:1-32](file://med_ai_assistant_1.0_bs_backend/config/application-prompt.properties#L1-L32)
- [PromptServiceConfig.java:1-308](file://med_ai_assistant_1.0_bs_backend/src/main/java/com/example/medaiassistant/config/PromptServiceConfig.java#L1-L308)
- [PromptServiceConfigTest.java:1-185](file://med_ai_assistant_1.0_bs_backend/src/test/java/com/example/medaiassistant/config/PromptServiceConfigTest.java#L1-L185)
- [PromptServiceConfigSimpleTest.java:1-164](file://med_ai_assistant_1.0_bs_backend/src/test/java/com/example/medaiassistant/config/PromptServiceConfigSimpleTest.java#L1-L164)
- [PromptServiceConfigIntegrationTest.java:1-125](file://med_ai_assistant_1.0_bs_backend/src/test/java/com/example/medaiassistant/config/PromptServiceConfigIntegrationTest.java#L1-L125)

## 依赖分析
- 配置导入链
  - main-linux-oracle 的 application.properties 通过 spring.config.import 导入 ai-models.properties。
  - 两个环境的 application.properties 均被引入监控、Prompt与医院配置。
  - **语音识别配置**通过VoiceRecognitionProperties配置类实现统一管理。
  - **定时任务配置**通过SchedulingProperties配置类实现统一管理。
  - **监控配置映射**通过MonitoringProperties配置类实现统一管理。
  - **Prompt服务配置**通过PromptServiceConfig配置类实现R-003性能优化。
...
- 参数耦合
  ...
  - **Prompt服务配置**通过配置类实现R-003性能优化，支持页面大小、批处理大小的动态调整。
...
DEV_APP --> TXN_LEVEL
PROMPT_CLASS --> PROMPT
PROMPT_CLASS --> ORA_APP
PROMPT_CLASS --> TEST_APP
PROMPT_CLASS --> DEV_APP
CLI_BACKEND --> MEMORY_BANK
CLI_FRONTEND --> CLI_BACKEND
THINK_PROTOCOL --> MEMORY_BANK
```
...
- 线程池与调度
  - 专用线程池（Prompt生成、手术分析）与通用执行器的队列容量与命名前缀有助于隔离与定位性能瓶颈。
  - 定时任务的执行时间与并发数应与下游服务能力匹配。
  - **定时任务配置优化**：通过enabled参数控制定时任务的启用状态，避免不必要的资源消耗。
  - **R-003性能优化**：通过增加页面大小和批处理大小，显著提高数据处理吞吐量。
- 缓存与内存
...
- 监控与日志
  ...
  - **监控配置优化**：通过MonitoringProperties配置类提供更精细的监控参数管理。
- **语音识别性能**
  - 通过配置外化支持不同ASR服务提供商，可根据网络状况和成本选择最优方案。
  - 合理设置采样率和音频格式以平衡识别精度与传输效率。
  - **baseUrl和apiKey优化**：生产环境使用阿里云DashScope API，提升服务稳定性和性能。
- **CLI规则性能影响**
  - Memory Bank框架通过系统化的文档化管理，减少重复劳动，提升开发效率。
  - 深度思考协议确保复杂问题的系统化分析，避免性能陷阱。
  - **配置类验证**：通过PromptServiceConfig等配置类的验证机制，确保配置的正确性和有效性。

**更新** 新增R-003性能优化和配置类验证的性能考虑，反映配置管理对系统性能的重要影响。
...
- **Prompt服务异常（R-003优化后）**
  - 现象：提交/轮询失败、队列积压、超时、吞吐量不足。
  - 排查：检查 application-prompt.properties 的优化参数（提交页面大小20、轮询页面大小50、轮询批处理大小100）；确认下游服务可用性。
  - **配置类验证**：检查PromptServiceConfig配置类的验证结果，确保R-003优化参数正确应用。
  - **性能监控**：通过配置摘要方法监控实际的页面大小和批处理大小是否符合预期。
- 医院集成失败
...
- **配置类验证问题**
  - 现象：配置类无法正确加载、参数绑定失败、配置验证失败。
  - 排查：检查@EnableConfigurationProperties注解是否正确配置；验证配置类的@ConfigurationProperties注解；确认测试用例中的@TestPropertySource配置正确。
  - **测试验证**：通过PromptServiceConfigTest、PromptServiceConfigSimpleTest、PromptServiceConfigIntegrationTest等测试用例验证配置类的正确性。

**更新** 新增R-003性能优化故障排查指导和配置类验证故障排查，反映新增的配置优化功能对系统稳定性的影响。
...
### R-003性能优化最佳实践
- **配置参数验证**：通过PromptServiceConfig配置类的验证机制，确保R-003优化参数的正确应用。
- **性能监控**：利用配置摘要方法监控实际的页面大小和批处理大小，确保优化效果。
- **测试验证**：通过完整的测试套件验证配置类的正确性和性能优化效果。
- **环境差异化**：开发环境和生产环境的配置参数应保持一致性，避免环境差异导致的问题。

**更新** 详细介绍Memory Bank框架、CLI规则最佳实践和深度思考协议，以及R-003性能优化的最佳实践，为开发者提供系统化的开发指导。
...
- **配置类验证**：通过@EnableConfigurationProperties注解确保配置类的正确加载和验证。

### 配置管理流程
- **规则制定**：通过CLI规则文档明确开发规范和最佳实践。
- **执行监控**：Memory Bank框架确保规则的有效执行和文档化。
- **持续改进**：深度思考协议促进开发过程的反思和优化。
- **配置验证**：通过配置类的验证机制确保配置的正确性和有效性。

### 开发效率提升
- **减少重复劳动**：通过系统化的规则和流程，避免重复性工作。
- **提升代码质量**：通过深度思考协议确保复杂问题的正确分析和解决。
- **知识传承**：Memory Bank框架确保团队知识的有效传承。
- **配置一致性**：通过配置类验证机制确保不同环境间配置的一致性。

**更新** 详细介绍CLI规则与配置管理的集成，展示如何通过系统化的规则和流程提升开发效率和代码质量，以及配置类验证在其中的重要作用。
...
## 结论
MedAiAssistant 的配置体系采用"环境分层 + 组件细分"的组织方式，通过 Spring Boot 的配置导入机制实现模型配置的集中管理，辅以 JSON/Properties 文件对缓存、内存与 AI 模型知识库进行精细化控制。**最新的配置优化**包括语音识别配置外化、定时任务配置控制、生产环境配置修复、数据库配置优化、JpaTransactionManager日志级别标准化、监控配置映射类的引入以及**R-003性能优化配置**，进一步提升了系统的灵活性、稳定性和安全性。

**更新** 本次更新重点体现了配置管理的持续改进：通过VoiceRecognitionProperties实现语音识别配置外化，通过SchedulingProperties实现定时任务配置控制，通过MonitoringProperties实现监控配置映射，通过PromptServiceConfig实现R-003性能优化，通过修复生产环境baseUrl和apiKey解决了语音识别服务问题，通过Oracle数据库PGA内存调整优化了数据库性能，通过JpaTransactionManager日志级别标准化消除了多环境间的配置差异。**新增的CLI规则与Memory Bank框架**以及**配置类验证机制**进一步完善了项目的配置管理体系，通过系统化的开发者最佳实践指南和深度思考协议，确保开发过程的规范化和高效化。

**R-003性能优化**是本次更新的核心亮点，通过对提交页面大小、轮询页面大小、批处理大小的系统性优化，显著提升了系统的吞吐量和响应效率。PromptServiceConfig配置类的引入实现了配置参数的完整映射和验证，确保了配置的正确性和可维护性。

建议在开发、测试、生产三类环境中遵循统一的优先级与安全策略，确保配置变更可追溯、敏感信息受控、性能与稳定性兼顾。同时，充分利用CLI规则、Memory Bank框架和配置类验证机制，提升开发效率和代码质量。
...
- 层级顺序（从高到低）
  ...
  5) **配置类默认值**（VoiceRecognitionProperties、SchedulingProperties、MonitoringProperties、PromptServiceConfig等Java配置类）
  6) **CLI规则与最佳实践**（Memory Bank框架和深度思考协议）
- 说明
  - 环境变量可用于覆盖任何配置项，便于在不同部署场景快速切换。
  - 环境专属配置用于承载与部署环境强相关的参数（如执行服务器地址、端口、下载目录等）。
  - 通用配置用于跨环境共享的参数（如监控、Prompt服务）。
...
- 建议
  ...
  - **Prompt服务配置**：通过PromptServiceConfig配置类提供配置验证和摘要功能，确保R-003优化参数的正确应用。
  - **事务日志配置**：保持JpaTransactionManager日志级别为WARN，避免敏感信息泄露。
  - **CLI规则管理**：通过Memory Bank框架确保敏感信息处理的规范性和可追溯性。

**更新** 新增定时任务配置、监控配置映射、Prompt服务配置和CLI规则在安全管理方面的考虑，强调Memory Bank框架在敏感信息管理中的作用。
...
- **Prompt服务（R-003优化）**
  - **提交服务**：提交页面大小20、最大线程数5、重试次数3、连接/读取超时300秒
  - **轮询服务**：轮询页面大小50、批处理大小100、重试次数5、超时30秒
  - **监控服务**：健康检查超时10秒、指标收集间隔30秒、错误率阈值5%、响应时间阈值10秒、队列长度阈值100
  - **配置类映射**：通过PromptServiceConfig实现完整的参数映射和验证
- 医院集成
...
- **CLI规则与Memory Bank**
  ...
  - **深度思考协议**：系统化问题分析和解决框架
  - **配置类验证**：通过@EnableConfigurationProperties确保配置类的正确加载和验证

**更新** 新增R-003性能优化配置、配置类验证和CLI规则与Memory Bank的配置项，全面反映最新的配置管理优化成果。