...
## 第2.5步：按钮帮助文档同步检查（前端）
如果本次变更涉及前端页面按钮或功能（新增/删除/修改按钮、按钮文本变更、data-help-key 变更、按钮行为/可用性逻辑变更），**在提交前**执行按钮帮助文档同步：
- 完整工作流见 `.qoder/commands/sync_help_docs.md`（或调用 `/sync-help-docs` 技能）
- 同步修改目标前端的 `src/help/entries/` 帮助条目（AIMedTeach：`AiMedTeach/med-teach-frontend/`；主系统 MedAI：`med_ai_assistant_1.0_bs_vue/`），并将帮助文档变更一并纳入本次提交与更新日志

> ⚠️ **适用前提**：本步骤仅适用于**已实现按钮帮助系统**的前端。当前两个前端均已实现（AIMedTeach 前端 2026-08-29、主系统 MedAI 前端 2026-08-31）；若变更来自未实现帮助系统的前端则跳过本步骤（如需实现帮助系统，另行评估）。

检查点：确认目标前端已实现帮助系统且前端有按钮/功能变更时帮助条目已同步更新（构建/测试全绿），或确认无相关变更/未实现帮助系统可跳过。
