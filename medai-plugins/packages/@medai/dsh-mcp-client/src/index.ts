...
/** 模块级 JWT provider（A2 fork：工作站登录人 JWT，每请求动态头）。 */
let jwtProvider: (() => string | null) | null = null

/**
 * 设置 JWT provider（工作站登录人身份；N2 client 侧 setCredentials 注入）。
 * @param provider 返回当前登录人 JWT（null/登出 → 回退静态 Authorization）；
 *                 传 null 清除。STATELESS 网关无会话表，换人仅 JWT 头变化，
 *                 无需重建连接（S2 fork 实现 2 简化版）。
 */
export function setCredentials(provider: (() => string | null) | null): void {
  jwtProvider = provider
}

/** 当前 JWT（transport 每请求 getter 调用）。 */
